...
def test_fix_hidden_by_aistton_both(gram_checker, errors, wanted_errors):
    assert gram_checker.fix_hidden_by_aistton(errors) == wanted_errors
...
def test_fix_aistton_both(gram_checker, errors, wanted_errors):
    my_errors = gram_checker.fix_aistton(errors)
    assert list(my_errors) == wanted_errors


@pytest.mark.parametrize(
    ("paragraph", "want_parts", "want_errors"),
    [
        (
...
def test_extract_error_info(gram_checker, paragraph, want_parts, want_errors):
    parts = []
    errors = []
    gram_checker.extract_error_info(parts, errors, etree.fromstring(paragraph))
